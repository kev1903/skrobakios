...
-- Add additional security logging without compromising access control
-- This function will log access attempts without bypassing security
CREATE OR REPLACE FUNCTION log_lead_access_attempt()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
BEGIN
  -- Log successful lead access for security monitoring
...
-- Create trigger for secure logging (only on successful access)
...
CREATE TRIGGER log_lead_access_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION log_lead_access_attempt();

-- Add function to mask sensitive contact info for non-company members
...
BEGIN
  ...
  RETURN QUERY SELECT
    ...
    CASE 
      ...
      ELSE regexp_replace(lead_record.contact_email, '^(.{2})[^@]*(@.*)$', '\1***\2')
    END as contact_email,
    CASE 
      WHEN is_company_member THEN lead_record.contact_phone
      ELSE regexp_replace(lead_record.contact_phone, '^(.{3}).*(.{2})$', '\1***\2')
