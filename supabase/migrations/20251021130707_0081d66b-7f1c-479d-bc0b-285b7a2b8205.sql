-- Fix user details display by falling back to auth.users metadata when profiles are missing
...
BEGIN
  ...
  IF requester_is_superadmin THEN
    ...
    SELECT DISTINCT ON (au.id)
      ...
      -- Fallback to raw_user_meta_data if profile is missing
      COALESCE(p.first_name, au.raw_user_meta_data->>'first_name') AS first_name,
      COALESCE(p.last_name, au.raw_user_meta_data->>'last_name') AS last_name,
      p.avatar_url,
      p.phone,
      c.name AS company,
      COALESCE(
...
-- Backfill missing profile records from auth.users metadata
