-- Insert default platform settings with proper JSONB casting
INSERT INTO public.platform_settings (setting_key, setting_value, setting_type, description) 
SELECT * FROM (VALUES
  ('site_name', '"Lovable Platform"'::jsonb, 'general', 'Name of the platform'),
  ...
  ('max_companies_per_user', '5'::jsonb, 'limits', 'Maximum companies a user can create'),
  ('max_projects_per_company', '100'::jsonb, 'limits', 'Maximum projects per company'),
  ('default_subscription_plan', '"FREE"'::jsonb, 'subscription', 'Default subscription plan for new users'),
  ('email_notifications_enabled', 'true'::jsonb, 'notifications', 'Enable email notifications'),
  ('api_rate_limit', '{"requests_per_minute": 60, "burst_limit": 100}'::jsonb, 'api', 'API rate limiting configuration'),
  ('file_upload_limits', '{"max_file_size_mb": 50, "allowed_types": ["pdf", "doc", "docx", "jpg", "png"]}'::jsonb, 'files', 'File upload restrictions'),
  ('security_settings', '{"password_min_length": 8, "require_2fa": false, "session_timeout_hours": 24}'::jsonb, 'security', 'Security configuration')
