-- Step 1: Create new enum with renamed roles
CREATE TYPE app_role_new AS ENUM ('superadmin', 'platform_admin', 'company_admin');

-- Step 2: Remove default value temporarily
ALTER TABLE public.user_roles ALTER COLUMN role DROP DEFAULT;

-- Step 3: Update the user_roles table column to use new enum
ALTER TABLE public.user_roles 
ALTER COLUMN role TYPE app_role_new 
USING CASE 
  WHEN role::text = 'admin' THEN 'platform_admin'::app_role_new
  WHEN role::text = 'user' THEN 'company_admin'::app_role_new
  ELSE role::text::app_role_new
END;

-- Step 4: Set new default value
ALTER TABLE public.user_roles ALTER COLUMN role SET DEFAULT 'company_admin'::app_role_new;

-- Step 5: Drop old enum and rename new one
DROP TYPE public.app_role;
ALTER TYPE app_role_new RENAME TO app_role;

-- Step 6: Update all database functions with new role names
