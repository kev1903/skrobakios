-- Fix Critical Security Issue: Secure Estimates and Pricing Data (Fixed)
...
-- 6. Drop existing policies on estimate_line_items if they exist
DROP POLICY IF EXISTS "Company members can view estimate line items" ON public.estimate_line_items;
DROP POLICY IF EXISTS "Company members can manage estimate line items" ON public.estimate_line_items;

-- Create secure policies for estimate_line_items
...
-- 7. Create trigger function and trigger for estimate_line_items updated_at (handle existing)
