...
export const LandingPage = ({ onNavigate }: LandingPageProps) => {
  ...
  return (
    <div className="h-screen overflow-hidden relative bg-brand-white">
      ...
      <div className="fixed right-1.5 sm:right-3 lg:right-4 top-1/2 transform -translate-y-1/2 z-40 flex flex-col gap-0.5 sm:gap-1.5 bg-brand-white/40 sm:bg-brand-white/60 backdrop-blur-sm rounded-full py-0.5 sm:py-1 px-0.5 sm:px-1">
        {slides.map((slide, index) => (
          <button
            key={slide.id}
            onClick={() => goToSlide(index)}
            aria-label={`Go to ${slide.title}`}
            className="group p-0.5 sm:p-1 m-0 border-0 bg-transparent cursor-pointer"
          >
            <span
              className={`block w-1 h-1 sm:w-2 sm:h-2 rounded-full transition-transform duration-300 ${
                currentSlide === index 
