  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Active state - steel blue theme to match the geometric design  
        isActive && "bg-gradient-to-br from-blue-400 via-slate-500 to-slate-600 backdrop-blur-xl border border-white/20 shadow-2xl",
        ...
        // Active hover effects - steel blue theme
        onClick && isActive && "hover:shadow-[0_0_40px_rgba(71,85,105,0.6)] hover:scale-110 hover:border-white/40 focus:ring-slate-400/30 focus:ring-offset-2",
        (isActive || isListening) && "shadow-[0_0_50px_rgba(71,85,105,0.8)] ring-4 ring-slate-300/50",
      ...
      style={{
        ...
        boxShadow: isActive && isSpeaking 
          ? `0 0 ${40 + getGlowIntensity() * 60}px rgba(71, 85, 105, ${0.6 + getGlowIntensity() * 0.4}), 
             0 0 ${80 + getGlowIntensity() * 40}px rgba(51, 65, 85, ${0.3 + getGlowIntensity() * 0.3}),
             inset 0 2px 4px rgba(255, 255, 255, 0.3)` 
          : isActive 
            ? "0 20px 40px rgba(71, 85, 105, 0.4), 0 10px 20px rgba(51, 65, 85, 0.3), inset 0 2px 4px rgba(255, 255, 255, 0.3)"
    ...
    >
      ...
      {/* Enhanced inner glow layer - only when active */}
      {isActive && (
        <div 
      ...
      {/* Sophisticated glossy highlight - only when active */}
      {isActive && (
        <div 
          className="absolute inset-2 rounded-full opacity-80"
          style={{
            background: "linear-gradient(135deg, rgba(255, 255, 255, 0.9) 0%, rgba(255, 255, 255, 0.4) 25%, rgba(71, 85, 105, 0.1) 50%, transparent 75%)"
          }}
        />
      )}

      {/* Dynamic particles effect - only when active */}
      {isActive && (
        <div className="absolute inset-0 rounded-full overflow-hidden">
          ...
          <div className="absolute w-0.5 h-0.5 bg-slate-200/80 rounded-full animate-ping" style={{bottom: '30%', left: '60%', animationDelay: '2s'}} />
        </div>
      )}

      {/* Enhanced listening glow */}
      {isListening && !isSpeaking && (
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-slate-400/40 via-slate-600/30 to-white/20 animate-pulse" />
      )}

      {/* Sophisticated outer ring animation for listening state */}
      {isListening && !isSpeaking && (
        <div className="absolute inset-0 rounded-full border-2 border-slate-300/50 animate-ping" />
      )}

      {/* Enhanced audio level visualization ring */}
      {audioLevel > 0 && isListening && (
        <div 
          className="absolute inset-0 rounded-full border-2 border-gradient-to-r from-slate-400 to-slate-600 transition-all duration-100 shadow-lg"
          style={{
            transform: `scale(${1 + audioLevel * 0.3})`,
            opacity: audioLevel * 0.8 + 0.2,
            boxShadow: `0 0 ${10 + audioLevel * 20}px rgba(71, 85, 105, ${audioLevel * 0.6})`
          }}
        />
      )}
      ...
      {onClick && (
        <div className={cn(
          ...
          isActive && isSpeaking ? "bg-gradient-to-r from-slate-400 to-slate-600 animate-pulse shadow-slate-400/50" : 
          isActive && isListening ? "bg-gradient-to-r from-slate-400 to-slate-700 animate-pulse shadow-slate-400/50" :
          isActive ? "bg-gradient-to-r from-slate-300 to-white shadow-slate-300/50" : "bg-gradient-to-r from-gray-400 to-gray-500"
        )} />
      )}
    </div>
  );
