...
export const RoleManagement: React.FC<RoleManagementProps> = ({
  ...
  const handleAddRole = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await onAddRole(user.id, selectedRole);
      if (result.success) {
        toast({
          title: 'Role Added',
  ...
  const handleRemoveRole = async (role: UserRole) => {
    ...
    setIsLoading(true);
    try {
      const result = await onRemoveRole(user.id, role);
      if (result.success) {
        toast({
          title: 'Role Removed',
        ...
        {user.roles.map(role => (
          <div key={role} className="flex items-center gap-1">
            <Badge variant={roleHierarchy[role].variant}>
              ...
              {role === user.role && <span className="ml-1 text-xs">(Primary)</span>}
            </Badge>
            {user.roles.length > 1 && role !== 'superadmin' && (
              <Button
                variant="ghost"
                size="sm"
