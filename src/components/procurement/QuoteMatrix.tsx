            ...
            {wbsMatrix.map((row, index) => (
              ...
              >
                ...
                {[0, 1, 2, 3, 4].map((contractorIndex) => {
                  ...
                  return (
                    <div key={contractorIndex} className={`col-span-1 text-center text-sm font-medium text-gray-900 self-center px-2 py-3 h-full border-l border-gray-200 ${isBlueColumn ? 'bg-blue-50 hover:bg-blue-100' : 'hover:bg-gray-100'} relative group`}>
                      <div className="flex items-center justify-center space-x-2">
                        <span>{contractor?.quote ? formatCurrency(contractor.quote) : ''}</span>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-5 w-5 p-0 rounded-full bg-primary text-primary-foreground hover:bg-primary/80 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
