...
import { WBSItemInput } from '@/types/wbs';
...
export const ProjectScopePage = ({ project, onNavigate }: ProjectScopePageProps) => {
    ...
    switch (action) {
      ...
      case 'insert-below':
        ...
        try {
          ...
          console.log('🔵 Insert below item:', { itemId, item, currentItemIndex, level: item.level });
          ...
          if (item.level === 0) {
            ...
            const maxRootNum = Math.max(...rootItems.map(i => {
              const firstPart = i.wbs_id?.split('.')[0];
              return parseInt(firstPart || '0');
            }), 0);
            ...
            const wbsPrefix = wbsParts.slice(0, -1).join('.');
  ...
  }, [flatWBSItems, updateWBSItem, createWBSItem, deleteWBSItem, loadWBSItems, currentCompany, project, taskHandlers]);
  ...
  const contentClasses = {
    mobile: "fixed inset-x-0 top-[var(--header-height)] bottom-0 overflow-hidden",
    tablet: "fixed inset-x-0 top-[var(--header-height)] bottom-0 overflow-hidden",
    desktop: "fixed left-40 right-0 top-[var(--header-height)] bottom-0 overflow-hidden"
  };
