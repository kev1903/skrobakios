...
export const ProjectScopePage = ({
  ...
  console.log('ProjectScopePage rendering', {
    project,
  ...
  const scopeData: ScopePhase[] = wbsItems.filter(item => item.wbs_id?.endsWith('.0') || item.level === 0 || item.parent_id == null).sort((a, b) => {
    ...
    components: phase.children?.filter(child => child.level === 1 || child.wbs_id && !child.wbs_id.endsWith('.0') && child.wbs_id.split('.').length === 2).map(component => ({
      id: component.id,
      ...
      elements: component.children?.filter(child => child.level === 2 || child.wbs_id && child.wbs_id.split('.').length === 3).map(element => ({
        id: element.id,
        name: element.title,
        ...
        tasks: element.children?.filter(child => child.level === 3 || child.category === 'Task').map(task => ({
          id: task.id,
          name: task.title,
  ...
  const flatWBSItems = React.useMemo(() => {
    ...
    scopeData.forEach((phase, phaseIndex) => {
      items.push({
        ...
        start_date: null,
        // Add from WBS item if available
        end_date: null,
        // Add from WBS item if available
        duration: 0,
        // Add from WBS item if available
    ...
    return items.map(item => {
      ...
      if (wbsItem) {
        return {
          ...
          name: wbsItem.title || item.name,
          // Use WBS title if available
          title: wbsItem.title || item.name,
          // Also set title field
  ...
  const togglePhase = async (phaseId: string) => {
    ...
    if (phase) {
      await updateWBSItem(phaseId, {
        is_expanded: !phase.is_expanded
      });
    }
  };
  const toggleComponent = async (_phaseId: string, componentId: string, currentExpanded: boolean) => {
    await updateWBSItem(componentId, {
      is_expanded: !currentExpanded
    });
  };
  ...
  const expandAll = async () => {
    ...
    for (const phase of scopeData) {
      ...
      for (const component of phase.components) {
        if (!component.isExpanded) {
          await updateWBSItem(component.id, {
            is_expanded: true
          });
        }
      }
  ...
  const saveNotes = async () => {
    ...
    try {
      await updateWBSItem(currentNotesItem.id, {
  ...
  const ProgressInput = ({
    ...
    if (disabled) {
      return <span className={`text-xs text-muted-foreground font-medium ${className}`} title="Calculated from child items">
          {value}%
        </span>;
    }
    const handleSave = () => {
    ...
    if (isEditing) {
      return <Input value={inputValue} onChange={e => setInputValue(e.target.value)} onBlur={handleSave} onKeyDown={handleKeyDown} className={`w-8 h-4 text-xs p-0 text-center border-0 bg-transparent focus:bg-white focus:border focus:ring-1 ${className} [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield]`} autoFocus type="number" min="0" max="100" step="1" pattern="[0-9]*" />;
    }
    return <span className={`text-xs text-muted-foreground font-medium cursor-pointer hover:bg-accent/20 rounded px-1 ${className}`} onClick={() => {
      setInputValue(value.toString());
      setIsEditing(true);
  ...
  const StatusSelect = ({
    ...
    if (disabled) {
      return <div className={`h-7 w-16 text-xs border-0 bg-transparent p-1 ${className}`}>
    ...
    return <Select value={value} onValueChange={onChange}>
        ...
        <SelectContent className="min-w-36 bg-background border shadow-lg z-50 p-1">
          {statusOptions.map(status => <SelectItem key={status.value} value={status.value} className="text-xs py-2 px-2 cursor-pointer">
      ...
      for (let phaseIndex = 0; phaseIndex < scopeData.length; phaseIndex++) {
        ...
        for (let compIndex = 0; compIndex < phase.components.length; compIndex++) {
          ...
          itemsToSave.push({
            ...
            project_id: project.id,
            parent_id: null,
            // Will be set to phase ID after creation
          ...
          for (let elemIndex = 0; elemIndex < component.elements.length; elemIndex++) {
            ...
            itemsToSave.push({
              ...
              project_id: project.id,
              parent_id: null,
              // Will be set to component ID after creation
              wbs_id: elementWbsId,
  ...
  const EditableCell = ({
    ...
    return <div ref={wrapperRef} className={`w-full h-full flex items-center ${!isEditing ? 'cursor-pointer hover:bg-accent/20 rounded px-1' : ''}`} onClick={() => !isEditing && setIsEditing(true)} data-field={field}>
        {isEditing ? <Input ref={localInputRef} value={localValue} onChange={e => setLocalValue(e.target.value)} onKeyDown={onKeyDown} onMouseDown={e => e.stopPropagation()} className="border-none outline-none focus:outline-none focus:border-none ring-0 focus:ring-0 focus-visible:ring-0 ring-offset-0 focus:ring-offset-0 focus-visible:ring-offset-0 shadow-none bg-transparent p-0 m-0 rounded-none h-auto w-full" placeholder={placeholder} autoFocus /> : <span className={`truncate ${className}`}>
            {value || placeholder}
          </span>}
      </div>;
  };
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    try {
      ...
      const flatItems = wbsItems.filter(item => item.level === 0 || item.parent_id == null).sort((a, b) => {
        const aWbs = a.wbs_id?.split('.').map(n => parseInt(n)) || [0];
        const bWbs = b.wbs_id?.split('.').map(n => parseInt(n)) || [0];
        return aWbs[0] - bWbs[0];
      });
  ...
  const calculateTimelineProgress = () => {
    ...
    const wbsEndDates = wbsItems.filter(item => item.end_date).map(item => new Date(item.end_date!));
    const projectEndDate = wbsEndDates.length > 0 ? new Date(Math.max(...wbsEndDates.map(d => d.getTime()))) : new Date(Date.now() + 90 * 24 * 60 * 60 * 1000); // Default to 90 days from now
  ...
  const contentClasses = {
    mobile: "fixed inset-x-0 top-12 bottom-0",
    tablet: "fixed inset-x-0 top-12 bottom-0",
    desktop: "fixed left-40 right-0 top-12 bottom-0"
  };
  return <div className={mainClasses[screenSize]}>
      <ProjectSidebar project={project} onNavigate={onNavigate} getStatusColor={getStatusColor} getStatusText={status => status} activeSection="specification" />
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full w-full flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <div className="w-24 h-1.5 bg-muted rounded-full overflow-hidden">
                          <div className={`h-full transition-all duration-300 ${getProgressColor(getProgressData().value)}`} style={{
                          width: `${getProgressData().value}%`
                        }} />
                        </div>
                    ...
                    <div className="flex items-center justify-between gap-6 flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        {activeTab === 'scope' && <Button size="sm" onClick={() => addNewPhase()}>
                            <Plus className="w-3 h-3 mr-1" />
                            Add Phase
                          </Button>}
                        {activeTab === 'time' && <div className="flex items-center gap-2">
                            <Button variant="outline" size="sm" onClick={() => {}} title="Critical Path">
                              <span className="text-xs font-medium">Critical Path</span>
                            </Button>
                          </div>}
                        {activeTab === 'cost' && <div className="flex items-center gap-2">
              ...
              <TabsContent value="scope" className="h-full w-full m-0 p-0 data-[state=active]:flex">
                <div className="h-full w-full flex flex-col">
                   {loading && wbsItems.length === 0 ? <div className="flex items-center justify-center h-64">
                    ...
                    overflowY: 'scroll'
                        ...
                        <WBSSplitView items={flatWBSItems} onToggleExpanded={itemId => {
                      const item = wbsItems.find(i => i.id === itemId);
                      if (item) {
                    ...
                    }} onDragEnd={onDragEnd} onItemUpdate={updateWBSItem} onAddChild={addChildItem} onContextMenuAction={handleContextMenuAction} onOpenNotesDialog={openNotesDialog} dragIndicator={dragIndicator} EditableCell={EditableCell} StatusSelect={StatusSelect} ProgressInput={ProgressInput} ProgressDisplay={ProgressDisplay} getProgressColor={getProgressColor} generateWBSNumber={generateWBSNumber} />
                       </div>}
                 </div>
               </TabsContent>
              ...
              <TabsContent value="time" className="h-full w-full m-0 p-0 data-[state=active]:flex">
                <div className="h-full w-full flex flex-col">
                  {loading && wbsItems.length === 0 ? <div className="flex items-center justify-center h-64">
                    ...
                    </div> : <div className="flex-1 min-w-0 overflow-y-auto">
                      <WBSTimeView items={flatWBSItems} onToggleExpanded={itemId => {
                      const item = wbsItems.find(i => i.id === itemId);
                      if (item) {
                        updateWBSItem(itemId, {
                          is_expanded: !item.is_expanded
                    ...
                    }} onDragEnd={onDragEnd} onItemUpdate={updateWBSItem} onAddChild={addChildItem} onContextMenuAction={handleContextMenuAction} onOpenNotesDialog={openNotesDialog} onClearAllDates={clearAllDates} dragIndicator={dragIndicator} EditableCell={EditableCell} StatusSelect={StatusSelect} generateWBSNumber={generateWBSNumber} />
                    </div>}
                </div>
              </TabsContent>
                 ...
                 <div className="h-full w-full flex flex-col">
                    ...
                    overflowY: 'scroll'
                      ...
                      WebkitOverflowScrolling: 'touch'
                        ...
                        }} onDragEnd={onDragEnd} onItemUpdate={updateWBSItem} onAddChild={addChildItem} onContextMenuAction={handleContextMenuAction} onOpenNotesDialog={openNotesDialog} dragIndicator={dragIndicator} EditableCell={EditableCell} StatusSelect={StatusSelect} generateWBSNumber={generateWBSNumber} />
                           </div>
                         </div>
                       </div>}
                  </div>
        ...
        <Dialog open={notesDialogOpen} onOpenChange={open => !open && closeNotesDialog()}>
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              ...
              <Textarea placeholder="Enter your notes here..." value={notesValue} onChange={e => setNotesValue(e.target.value)} className="min-h-[120px]" />
