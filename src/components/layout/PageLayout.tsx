...
export const PageLayout = ({ currentPage, onNavigate, children }: PageLayoutProps) => {
  ...
  if (currentPage === "system" || currentPage === "home" || currentPage === "tasks") {
    return (
      <ProtectedRoute 
        ...
        requireSuperAdmin={false}
      >
        <div className="w-full h-[calc(100vh-var(--header-height))] pt-[var(--header-height)] overflow-hidden">
          {children}
        </div>
      </ProtectedRoute>
    );
  ...
  return (
    ...
    >
      <main className={`flex-1 overflow-hidden w-full pt-[var(--header-height)] h-[calc(100vh-var(--header-height))]`}>
        {children}
      </main>
    </ProtectedRoute>
  );
