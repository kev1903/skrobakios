...
import { useQualityInspections } from '@/hooks/useQAQCData';
...
export const InspectionsPage = ({ onNavigate }: InspectionsPageProps) => {
  const [searchParams] = useSearchParams();
  const projectId = searchParams.get('projectId');
  const { getProject } = useProjects();
  const [project, setProject] = useState<Project | null>(null);
  
  const { data: inspections, isLoading } = useQualityInspections(projectId || '');
      ...
      <div className="flex-1 ml-48 h-screen overflow-y-auto bg-background">
        ...
        <div className="flex-shrink-0 border-b border-border px-6 py-4 bg-white backdrop-blur-sm">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-foreground font-inter">QA/QC - Quality Inspections & Tests</h1>
        ...
        <div className="p-6 min-h-full">
          <div className="w-full">
            <div className="bg-white rounded-lg shadow">
              <QAQCTable data={inspections || []} type="inspections" isLoading={isLoading} />
            </div>
          </div>
        </div>
      </div>
